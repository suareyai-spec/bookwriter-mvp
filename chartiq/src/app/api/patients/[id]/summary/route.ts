...
RECENT LABS:
${patient.labResults.slice(0, 20).map(l => `[${l.resultAt}] ${l.testName}: ${l.value} ${l.unit || ''} (${l.normalRange || ''}) ${l.flag === 'critical' ? '⚠️ CRITICAL' : l.flag === 'high' || l.flag === 'low' ? '⚡ ABNORMAL' : ''}`).join('\n')}
