...
export default function PatientDetailPage() {
  ...
  const saveNote = async () => {
    ...
    fetch(`/api/chartiq/patients/${id}`).then(r => r.json()).then(setPatient);
  };

  useEffect(() => {
    fetch(`/api/chartiq/patients/${id}`).then(r => r.json()).then((p: any) => {
      setPatient(p);
  ...
  const loadSummary = async () => {
    ...
    const res = await fetch(`/api/chartiq/patients/${id}/summary`);
  ...
  const sendChat = async () => {
    ...
    try {
      const res = await fetch(`/api/chartiq/patients/${id}/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveProfile = async () => {
    ...
    await fetch(`/api/chartiq/patients/${id}/profile`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const uploadImage = async (file: File, category: string, description: string) => {
    ...
    const res = await fetch(`/api/chartiq/patients/${id}/images`, { method: 'POST', body: fd });
  ...
  const deleteImage = async (imageId: string) => {
    await fetch(`/api/chartiq/patients/${id}/images/${imageId}`, { method: 'DELETE' });
