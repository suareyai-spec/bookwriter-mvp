...
export async function GET(req: NextRequest) {
  ...
  const subscriptions = users.map((u) => ({
    ...
    monthlyCreditsRemaining: (u.monthlyCredits ?? 0) + (u.creditsRollover ?? 0),
    packCreditsRemaining: u.purchasedCredits ?? 0,
    creditsUsedThisMonth: u.creditsUsedThisMonth ?? 0,
    creditsUsedAllTime: u.creditsUsedAllTime ?? 0,
